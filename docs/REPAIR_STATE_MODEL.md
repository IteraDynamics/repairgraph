...
| `tests/test_state_timeline.py` | implemented |
| `tests/test_state_replay.py` | implemented |
| `tests/test_state_export_mermaid.py` | implemented |
| `tests/test_state_visualization_payload.py` | implemented |
| `tests/test_state_visualization_api.py` | implemented |
| `tests/test_state_visualization_cli.py` | implemented |

Visualization modules:

| Module | Status |
|---|---|
| `state/timeline.py` | implemented |
| `state/replay.py` | implemented |
| `state/export_mermaid.py` | implemented |
| `state/visualization_payload.py` | implemented |
| `state/visualization_cli.py` | implemented |

## Replay architecture

State replay is provided by `state/replay.py`. It implements incremental
event-by-event state projection and structured state diffing.

### `replay_repair_state(initial_state, events) -> list[RepairState]`

Applies events one at a time to a deep copy of the initial state, collecting
one `RepairState` snapshot per event. The input state is never mutated.
Snapshots are deterministic, ordered, and side-effect free.

Each snapshot `i` contains `events[0..i]` in its event ledger and reflects
the accumulated state after those events have been applied.

### `build_state_diff(previous_state, current_state) -> dict`

Compares two `RepairState` snapshots and returns a structured dict of changed
entity types. Keys are only present when a change was detected.

Change detection covers:
- `session_status` — session status change
- `phases` — per-phase status changes (by `phase.name`)
- `actions` — per-action status changes (by `action_id`)
- `zones` — per-zone status changes (by `zone_id`)
- `qa_gates` — per-gate status changes (by `gate_id`)
- `blockers` — per-blocker status changes (by `blocker_id`)
- `next_recommended_actions` — added/removed action IDs

Returns an empty dict when the two states are identical.

### `summarize_state_diff(diff) -> dict`

Converts a raw diff dict into a human-readable summary with:
- `change_count` — total number of entity changes
- `changed_entities` — which entity types changed
- `changes` — list of human-readable change descriptions
- `has_*_change` — boolean flags per entity type

## Visualization architecture

Workflow visualization is provided by four modules:

| Module | Purpose |
|---|---|
| `state/timeline.py` | Ordered timeline views of events, phases, and actions |
| `state/export_mermaid.py` | Mermaid diagram generators |
| `state/visualization_payload.py` | Combined introspection/debug payload builder |
| `state/visualization_cli.py` | CLI that writes JSON and Mermaid files |

### Timeline model

Three timeline types are produced from a `RepairState`:

**Event timeline** (`build_event_timeline`) — ordered list of event ledger
entries with seq, event_id, timestamp, event_type, actor, target_type,
target_id, notes, and evidence. Ordered by event ledger append order.

**Phase timeline** (`build_phase_timeline`) — list of phase entries sorted by
phase number. Each entry includes status, zones, completed/pending/blocked
actions, related blockers, and advisory notes from blockers.

**Action timeline** (`build_action_timeline`) — list of action entries sorted
by (phase, action_id). Each entry includes status, zone refs, QA gate
associations, and evidence.

**Timeline summary** (`summarize_timeline`) — compact dict with event count,
phase status breakdowns, action status breakdowns, open QA gate count, open
blocker count, and next action IDs.

### Mermaid exports

Four Mermaid diagram types are generated by `state/export_mermaid.py`:

**`build_workflow_timeline_mermaid(state)`** — `sequenceDiagram` showing
events as messages from actor participants to target-type participants.
Each message is labelled with the event type and timestamp.

**`build_phase_flow_mermaid(state)`** — `flowchart LR` with phase nodes in
sequence, each labelled with its name and current status. Node colour reflects
status: amber=in_progress, green=complete, red=blocked, grey=not_started.

**`build_blocker_flow_mermaid(state)`** — `flowchart TD` with one node per
blocker and directed edges to what each blocker blocks. Open blockers in red,
resolved in green.

**`build_zone_activation_mermaid(state)`** — `flowchart LR` with one node per
zone, labelled with name, material classification, and status. Colour-coded
by activation state.

All Mermaid outputs are deterministic, advisory-commented, and include the
vehicle model context in the header comment.

### Introspection payloads

`build_workflow_visualization_payload(state)` combines all visualization
components into a single JSON-serializable dict:

```
schema_name: repairgraph.workflow_visualization
advisory: true
session: {...}
workflow_summary: {phase_count, action_count, ..., event_count}
timelines:
  summary: {...}
  events: [...]
  phases: [...]
  actions: [...]
replay_metadata:
  event_count, snapshot_count, event_ids, event_types, event_timestamps
visualization:
  sections: [workflow_timeline, phase_flow, blocker_flow, zone_activation]
  mermaid:
    workflow_timeline: "sequenceDiagram ..."
    phase_flow: "flowchart LR ..."
    blocker_flow: "flowchart TD ..."
    zone_activation: "flowchart LR ..."
active_context: {active_phase_ids, blocked_phase_ids, active_zone_ids, ...}
blockers: {...}
next_actions: {...}
```

This payload is intended for internal tooling, operational inspection, and
demos. It is not a UI payload and requires no browser rendering.
...
| `GET /internal/state/accord/replay` | Event-by-event replay with state snapshots and diffs |
